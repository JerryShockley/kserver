...
#ifndef __pld
    // Scott Trappe: added check to see if "__builtin_prefetch" is present, if
    // so use it instead of pld instruction. This fix is from the bug report:
    // http://eigen.tuxfamily.org/bz/show_bug.cgi?id=677
    //
#if __has_builtin(__builtin_prefetch)
#define __pld(x)    __builtin_prefetch(x)
#else	// __has_builtin
