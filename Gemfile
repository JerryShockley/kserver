source 'https://rubygems.org'
ruby '2.2.10'

gem 'rails', '4.2.6'
gem 'sass-rails', '~> 5.0'
# gem 'sass-rails', github: 'rails/sass-rails', branch: 'master'
# gem 'sass'
gem 'uglifier', '>= 1.3.0' 
gem 'coffee-rails', '~> 4.1.0' 
gem 'jquery-rails', '~> 4.3.1'
gem 'turbolinks', '~> 2.1.0'
gem 'jbuilder', '~> 2.0'
gem 'sdoc', ' 0.4.0',  :group => :doc
gem 'spring', '~> 2.0.2', :group => :development
gem 'devise', '~> 4.4.3'
gem 'foundation-rails', '~> 5.5.1.2'
gem 'pg', '~> 1.0.0'
gem 'pundit', '~> 1.1.0'
gem 'simple_form', '~> 3.5.1'
gem 'slim-rails', '~> 3.1.3'
gem 'unicorn', '~> 5.4.0'
gem 'unicorn-rails', '~> 2.2.1'
gem 'jquery-turbolinks', '~> 2.1.0'
gem 'ransack', '~> 1.8.8', github: 'activerecord-hackery/ransack'
gem 'kaminari', '~> 1.1.1'
gem 'virtus', '~> 1.0.5'
gem 'reform', '~> 2.2.4'
gem 'money-rails', '~> 1.10.0'
gem 'mina', '~> 1.2.3'
gem 'mina-unicorn', '~> 2.0.0',  require: false
gem 'mina-sidekiq', '~> 1.0.3',  require: false
gem 'foundicons-rails', '~> 0.2.0'
gem 'js_assets', '~> 0.1.2'
gem 'enumerize', '~> 2.2.2'
# gem 'axlxs', github: 'randym/axlsx'
# gem 'acts_as_xlxs'

group :development do
  gem 'better_errors', '~> 2.4.0'
  # gem 'pry-rescue'
  # gem 'pry-stack_explorer'
  gem 'binding_of_caller', :platforms=>[:mri_21]
  gem 'guard-bundler', '~> 2.1.0'
  gem 'guard-rails', '~> 0.8.1'
  gem 'guard-rspec', '~> 4.7.3'
  gem 'quiet_assets', '~> 1.1.0'
  gem 'rails_layout', '~> 1.0.42'
  gem 'rb-fchange', '~> 0.0.6',  :require=>false
  gem 'rb-fsevent', '~> 0.10.3',  :require=>false
  gem 'rb-inotify', '~> 0.9.10',  :require=>false
  gem 'annotate', '~> 2.7.2',  github: 'ctran/annotate_models',  require: false
end

group :development, :test do
  gem 'spring-commands-rspec', '~> 1.0.4'
  # gem 'pry-byebug'
  gem 'byebug', '~> 10.0.2'
  gem 'jasmine-rails', '~> 0.14.7'
  gem 'jasmine-jquery-rails', '~> 2.0.3'
  gem 'rspec-rails', '~> 3.7.2'
  gem 'factory_girl_rails', "~> 4.0"
  gem 'faker', '~> 1.8.7'
  
end

group :test do
  # gem 'capybara-screenshot'
  gem 'fuubar', '~> 2.3.1'
  gem 'poltergeist', '~> 1.17.0'
  gem 'capybara', '~> 2.18.0'
  gem 'database_cleaner', '~> 1.6.2'
  gem 'launchy', '~> 2.4.3'
  gem 'selenium-webdriver', '~> 3.11.0'
end
